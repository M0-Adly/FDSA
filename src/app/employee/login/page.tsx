...
export default function EmployeeLogin() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Smart Detection: If it's not a full email, treat it as an Employee ID
      if (!input.includes('@')) {
        email = `emp${input}@gov.eg`;
      }
  ...
  return (
    <div className="min-h-screen bg-[#080c1a] flex items-center justify-center p-4 relative overflow-hidden font-sans text-right" dir="rtl">
      {/* Background Glows */}
      ...
      <div className="w-full max-w-md z-10">
        <div className="text-center mb-10 animate-fade-in">
          ...
          <h1 className="text-4xl font-black text-white tracking-tight mb-2 uppercase italic">بوابة الإدارة</h1>
          <p className="text-white/30 text-sm font-bold tracking-widest">نظام إدارة الأزمات والكوارث</p>
        </div>
        ...
        <div className="bg-white/5 backdrop-blur-2xl border border-white/10 rounded-[2.5rem] p-10 shadow-2xl relative overflow-hidden">
          ...
          <div className="relative z-10">
            <div className="bg-amber-500/10 border border-amber-500/20 rounded-2xl p-4 mb-8 flex items-center gap-3">
              ...
              <p className="text-[11px] font-black text-amber-200/70 uppercase leading-tight tracking-wider">منطقة محظورة — للموظفين المصرح لهم فقط</p>
            </div>

            {error && (
              <div className="mb-6 p-4 bg-red-500/10 border border-red-500/20 rounded-2xl text-xs font-bold text-red-400 flex items-center gap-2 justify-end">
                <span>{error === 'Invalid login credentials' ? 'بيانات الدخول غير صحيحة' : error}</span>
            ...
            <form onSubmit={handleLogin} className="space-y-6">
              <div>
                <label className="block text-[10px] font-black text-white/30 uppercase tracking-[0.2em] mb-2 mr-1">رقم الموظف أو البريد الإلكتروني</label>
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="e.g. 1001 or admin@aast.com"
                    className="w-full bg-white/5 border border-white/10 text-white rounded-2xl pr-12 pl-4 py-4 focus:border-emerald-500 outline-none transition-all placeholder:text-white/10 font-bold text-right"
                  />
                </div>
              </div>

              <div>
                <label className="block text-[10px] font-black text-white/30 uppercase tracking-[0.2em] mb-2 mr-1">كلمة المرور</label>
                <div className="relative">
                  <div className="absolute right-4 top-1/2 -translate-y-1/2 text-white/20">
              ...
              >
                {loading ? (
                  ...
                  <>
                    ...
                    <svg className="w-4 h-4 rotate-180" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><polyline points="9 18 15 12 9 6"/></svg>
                  </>
                )}
              </button>
            </form>
