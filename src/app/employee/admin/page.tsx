    ...
    <div className="flex-1 flex items-center justify-center p-8 bg-[radial-gradient(circle_at_top_right,rgba(16,185,129,0.05),transparent)]">
      <div className="w-full max-w-xl animate-fade-in">
        ...
        <div className="mb-8 flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-black text-white tracking-tight">{t('admin_console')}</h1>
            <p className="text-white/40 text-sm font-bold uppercase tracking-widest mt-1">{t('staff_provisioning')}</p>
          </div>
        ...
        <div className="mt-8 bg-white/5 border border-white/10 backdrop-blur-xl rounded-3xl p-8 shadow-2xl relative overflow-hidden">
          <div className="relative z-10">
            <h2 className="text-lg font-black text-white/90 mb-6 flex items-center gap-2">
              ...
              {t('pending_approvals')} ({pendingUsers.length})
            </h2>
                  ...
                  <div key={user.id} className="p-6 bg-black/20 border border-white/5 rounded-2xl space-y-4">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex gap-2">
                        <button onClick={() => handleApproval(user.id, 'approved')} className="px-4 py-2 bg-emerald-500/20 text-emerald-400 rounded-xl text-xs font-bold hover:bg-emerald-500/30 transition">{t('approve_account')}</button>
                        <button onClick={() => handleApproval(user.id, 'rejected')} className="px-4 py-2 bg-red-500/10 text-red-400 rounded-xl text-xs font-bold hover:bg-red-500/20 transition">{t('reject')}</button>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4 pt-4 border-t border-white/5">
                      <div>
                        <p className="text-[10px] text-white/20 uppercase font-bold mb-1">{t('registration_date')}</p>
                        <p className="text-xs text-white/60">{new Date(user.created_at).toLocaleString()}</p>
                      </div>
                      <div>
                        <p className="text-[10px] text-white/20 uppercase font-bold mb-1">{t('auth_email')}</p>
                    ...
                    {/* ID Image */}
                    <div>
                      <p className="text-[10px] text-white/20 uppercase font-bold mb-2">{t('national_id_image')}</p>
                      {user.national_id_image_url ? (
                        <div className="relative group rounded-xl overflow-hidden border border-white/10 aspect-video bg-black/40">
                          ...
                          <a href={user.national_id_image_url} target="_blank" className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition flex items-center justify-center text-xs font-bold text-white">{t('click_view_full')}</a>
                        </div>
                      ) : (
                        <div className="p-4 bg-white/5 rounded-xl text-center text-[10px] text-white/30 italic">{t('no_image')}</div>
                      )}
                    </div>
                  </div>
                ))}
