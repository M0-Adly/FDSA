...
export function AuditSidebar() {
  ...
  return (
    <div className="glass-panel p-4 flex flex-col h-full shadow-lg border-t-4 border-indigo-500">
      <h3 className="text-sm font-bold flex items-center gap-2 mb-4 text-slate-600 dark:text-slate-400 uppercase tracking-widest">
        <Terminal className="w-4 h-4" /> System Audit Trail
      </h3>
          ...
          {auditLog.map((entry) => (
            ...
            >
              ...
              <div className="flex-1">
                <p className="text-xs font-semibold dark:text-slate-200">{entry.msg}</p>
                <p className="text-[10px] text-slate-400 font-mono mt-1">{entry.timestamp}</p>
              </div>
            </motion.div>
          ))}
          {auditLog.length === 0 && (
            <p className="text-xs text-slate-400 italic text-center mt-10">No logs captured yet.</p>
          )}
        </AnimatePresence>
      </div>
    </div>
